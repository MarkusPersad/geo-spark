    ...
    <HoverCardContent class="w-fit max-w-120 p-4 space-y-3">
      ...
      <DatePicker v-model.range="clockState.timeRange" mode="dateTime" is24hr class="w-full"
        ...
        <template v-slot="{ togglePopover }">
          <Button :disabled="clockState.timeRangeBorrowed || !clockState.isClock || clockState.realTime"
            variant="outline" @click="togglePopover" class="w-full justify-start">
            <CalendarIcon />
      ...
      <NumberField v-model="clockState.multiplier" :disabled="!clockState.isClock || clockState.realTime"
        id="multiplier" :defaultValue="1" class="w-full" :formatOptions="{
      ...
      <template class="flex items-center space-x-2">
        <Switch :disabled="clockState.timeRangeBorrowed || !clockState.isClock" v-model="clockState.realTime"
          id="Real" />
